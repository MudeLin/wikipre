...
try:
	conn = MySQLdb.connect(host='localhost',user='root',passwd='627116',db='wiki',charset = "utf8", use_unicode = True)
...
while True:
    ...
    for page in rePage.finditer(text):
        recordArticle(page.groupdict())  #返回以有别名的组的别名为键、以该组截获的子串为值的字典，没有别名的组不包含在内
	endIndex = page.end()
