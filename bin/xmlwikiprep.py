...
def read(f):
    ...
    for event, elem in cElementTree.iterparse(f):
        ...
        if elem.tag == "title":
        ...
        elif elem.tag == "{http://www.mediawiki.org/xml/export-0.8/}page":
            #doc["_id"] = int(elem.get("{http://www.mediawiki.org/xml/export-0.8/}id"))
            ...
            # Free the memory of the building tre
        elem.clear()
            if cnt % 1000 == 0:
    ...
    #print >> sys.stderr, stats()
f="../eewiki-20131123-pages-articles.xml"
print isgeneratorfunction(read)